...
class CubeSide:
    def __init__(self, side_name: SIDES, corners: Tuple[Point, ...]):
        ...
        self.polygon = Polygon(self.corners_coords)
    ...
    def draw(self, texture=None):
        ...
        self.drawn_quad = Quad(points=helpers.flatten(self.corners_coords), texture=texture)
    ...
    def transform(self):
        modified_coord_values = [
            ...
            for idx, coord in enumerate(helpers.flatten(self.corners_coords))
        ]

        anim = Animation(points=modified_coord_values, duration=0.4, transition='out_back')
        anim += Animation(points=helpers.flatten(self.corners_coords), duration=0.4, transition='in_back')
