...
class GraphicController:
    ...
    @staticmethod
    def make_gradient_texture(width=500, light_direction='left_to_right', brightness_increase=None, rotate=None, height=None, fill_color=None):
        if height is None:
            height = width
        ...
        if light_direction == 'fill':
            ImageDraw.floodfill(gradient, xy=(1, 1), value=fill_color)
        ...
        for x, y in itertools.product(range(width), range(height)):
            ...
            draw.line([start, end], (fill_color or (255, 255, 255)) + (color,), width=1)
